...
class PreviewWidget:
    ...
    def __init__(self):
        ...
        # Channel visibility - support both RGB and N-channel modes
        self.show_r = True
        self.show_g = True
        self.show_b = True

        # For N-channel images: which channels to display as RGB
        # Default: first 3 channels [0, 1, 2]
        self.rgb_mapping = [0, 1, 2] if self.num_channels >= 3 else list(range(self.num_channels))
        # Channel visibility for N-channel mode
        self.channel_visibility = [True] * self.num_channels
    ...
    def update_display(self):
        ...
        filename = filenames[self.current_index]
        ...
        needs_reload = (
            ...
            or cfg.normalisation.normalisation_method
            != BackendInterface.get_cached_normalisation_method()
        )

        if needs_reload and os.path.exists(filepath):
            try:
                ...
                # For N-channel images, use rgb_mapping to convert to displayable RGB
                rgb_map = self.rgb_mapping if self.num_channels > 3 else None
                self.original_image = display_image_normalisation(img, rgb_mapping=rgb_map)
            except Exception as e:
                logger.error(f"Error loading image {filepath}: {e}")
                return
        else:
            img = BackendInterface.get_image_at_index(self.current_index)
            # For N-channel images, use rgb_mapping to convert to displayable RGB
            rgb_map = self.rgb_mapping if self.num_channels > 3 else None
            self.original_image = display_image_normalisation(img, rgb_mapping=rgb_map)
    ...
    def _update_label(self, filename=None, score=None):
        ...
        normal_count, anomalous_count = BackendInterface.get_label_distribution()

        # Calculate newly annotated samples using cached method
        new_nominal, new_anomalous = BackendInterface.get_active_learning_counts()

        # Format the file name (shortened version)
        filenames = BackendInterface.get_filenames()
        scores = BackendInterface.get_scores()

        fname = filenames[self.current_index]
        ...
        sc = scores[self.current_index]
        total_len = BackendInterface.get_image_count() - 1
